...
### Governance — Phase GOV-1: Pre-Deployment Foundation
- **Priority:** P1 | **Est:** 1-1.5 days | **Doc:** [governance-systems-plan.md](governance-systems-plan.md)
- Three YAML registries, no code. (1) **Data-classification registry** — extends `schema-registry.yaml` with per-column flags (`pii`, `student_voice`, `safety_sensitive`, `ai_exportable`, `retention_days`, `basis`). (2) **Feature-flag registry** — `docs/feature-flags.yaml` with key/type/default/toggler/blast_radius/source; seeded from migration 077. (3) **Vendor/DPA registry** — `docs/vendors.yaml` covering Anthropic, Supabase, Voyage, Vercel (data sent, retention, DPA status, sub-processors). Unblocks Phase 7I Admin Controls scope + any school procurement conversation. Recommended insertion: parallel to Dimensions3 Phase 7B. Created 14 Apr 2026.
