...
> Last updated: 21 April 2026 (**Preflight Phase 2A SHIPPED — Checkpoint 2.1 PASSED** — Python scanner worker `fab-scanner/` deployed to Fly.io (`preflight-scanner`, SYD, 512MB). All 17 STL rules implemented (R-STL-01..17), ruleset `stl-v1.0.0`. 11 commits on origin/main (`5e00518..262ae0c`). **116 pytest tests** (new, all passing), `npm test` baseline 1409 untouched. Production smoke tests on 4 fixtures passed: small-cube-25mm (known-good, 2.6s scan, 0 rules), seahorse-not-watertight (29k faces — R-STL-01 BLOCK + R-STL-04 WARN), chess-pawn-inverted-winding (R-STL-02 + R-STL-05 BLOCK), whale-not-watertight (R-STL-01 + R-STL-04). Full report: `preflight-phase-2a-checkpoint-2-1.md`. WIRING flipped `preflight-scanner` status → `deployed`. 4 follow-ups opened: FU-SCANNER-OOM (resolved — bumped 256→512MB during checkpoint), FU-SCANNER-SIDECAR-DRIFT (P3), FU-SCANNER-LEASE-REAPER (P2 — required before horizontal scaling), FU-SCANNER-EMAIL-VERIFY (P3). Phases 1A + 1B-1 + 1B-2 also SHIPPED (20–21 Apr) — 11 migrations (093-103), 7 tables, 12 seeded machines, Fabricator auth (Argon2id + opaque tokens), 3 private Storage buckets, AI guardrails in admin_settings, Resend email helper with notifications_sent idempotency, teacher Fabricator admin page, student fabrication_notify_email opt-out. api-registry 296→306 routes. Phase 2-0b cleared Gate B (53 fixtures bucketed + sidecared). Next: Preflight Phase 2B (SVG rules R-SVG-01..15 — same worker, new module). Previous session — 15 April: ShipReady Phase 1B teacher onboarding flow live.)
