...
> Last updated: 21 April 2026 (**Preflight Phases 1A + 1B-1 + 1B-2 SHIPPED** — 11 migrations (093-103) applied to prod, 7 new tables with RLS, 12 seeded machine profiles, Fabricator (lab tech) auth with cookie session pattern matching `student_sessions`, 3 private Storage buckets with service-role RLS, AI enrichment guardrails (kill switch + $5/day cap + tier1-only) seeded into admin_settings. Phase 1B-1 added additive columns for pre-check JSONB + printing sub-state + email idempotency (098), invite-session flag (099), student email opt-out (100), per-scan Haiku cost + multi-angle thumbnail metadata (101), Storage buckets (102), AI guardrail seed (103). 17 commits on origin/main across the day. Schema-registry + WIRING synced (preflight-pipeline now references 4 tables; preflight-scanner depends_on admin_settings). Baseline 1362 tests passing. Lesson #51 captured during 1A. Next: Preflight Phase 1B-2 (teacher Fabricator-invite UI, `/fab/login`, `/fab/set-password`, email dispatch) OR Phase 2 scanner worker. Previous session — 15 April: ShipReady Phase 1B teacher onboarding flow live. Migrations 083 + 084, 4-step welcome wizard, branded auth email templates. Teacher password recovery hardened (PKCE vs implicit split). Dimensions3 declared SHIPPED 15 Apr; polish to backlog.)
...
- **Status:** 🔴 ACTIVE — Phases 1A + 1B-1 + 1B-2 SHIPPED to prod (20–21 Apr 2026). Phase 2 (scanner worker) next.
- **Priority:** P0 (high teacher pain point — DT labs' #1 reported bottleneck) | **Est remaining:** ~11–14 days | **Docs:** [fabrication-pipeline.md](fabrication-pipeline.md) (master spec) + [preflight-phase-1a-brief.md](preflight-phase-1a-brief.md) + [preflight-phase-1b-1-brief.md](preflight-phase-1b-1-brief.md) (last completed brief) + [fabrication/phase-0-decisions.md](fabrication/phase-0-decisions.md) (D-01 through D-12 decisions log)
...
- **Phase 2 next (~2–3 days):** Python scanner worker on Fly.io. Requires fixtures bucketed + sidecar YAML first (Gate B) — fixtures currently an unsorted pile in `docs/projects/fabrication/fixtures/`. Worker reads `admin_settings.preflight.*` before dispatching Haiku enrichment; writes `ai_enrichment_cost_usd` + `thumbnail_views` per revision.
