...
> Last updated: 20 April 2026 (**Preflight Phase 1A SHIPPED** — new project, fabrication submission pipeline for DT labs. 5 migrations (093-097) applied to prod, 7 new tables with RLS, 12 seeded machine profiles, Fabricator (lab tech) auth tables with cookie session pattern matching `student_sessions`. 3 new WIRING systems (`preflight-pipeline`, `preflight-scanner`, `machine-profiles`). Schema-registry synced. Lesson #51 captured: Supabase dashboard "Run and enable RLS" popup mis-parses PL/pgSQL `DECLARE` variable names as table identifiers. 10 commits on origin/main. Baseline 1362 tests passing (was 1254 on 14 Apr). Next: Preflight Phase 1B (Storage buckets + retention cron + Fabricator invite UI) OR Dimensions3 Phase 7. Previous session — 15 April: ShipReady Phase 1B teacher onboarding flow live. Migrations 083 + 084, 4-step welcome wizard, branded auth email templates. Teacher password recovery hardened (PKCE vs implicit split). Dimensions3 declared SHIPPED 15 Apr; polish to backlog.)
...
- **Status:** 🔴 ACTIVE — Phase 1A SHIPPED to prod (20 Apr 2026). Phase 1B next.
- **Priority:** P0 (high teacher pain point — DT labs' #1 reported bottleneck) | **Est remaining:** ~13–16 days | **Docs:** [fabrication-pipeline.md](fabrication-pipeline.md) (master spec) + [preflight-phase-1a-brief.md](preflight-phase-1a-brief.md) (last completed brief) + [fabrication/phase-0-decisions.md](fabrication/phase-0-decisions.md) (D-01 through D-12 decisions log)
...
- **Phase 1B next (~2–3 days):** Storage buckets (`fabrication-uploads/`, `fabrication-thumbnails/`, `fabrication-pickup/`) + bucket policies + retention cron stub + teacher Fabricator-invite UI + `/fab/login` page.
- **Phase 2 next after (~2–3 days):** Python scanner worker on Fly.io. Requires fixtures bucketed + sidecar YAML first (Gate B) — fixtures currently an unsorted pile in `docs/projects/fabrication/fixtures/`.
