...
- **Status:** 🔴 ACTIVE — Phase 1A + 1B-1 SHIPPED to prod (20 Apr 2026). Phase 1B-2 next.
...
- **Phase 1B-2 next (~2–3 days):** Teacher Fabricator-invite UI, `/fab/login` cookie-session login page, `/fab/set-password` consuming `is_setup=true` sessions, email dispatch at status transitions (using `notifications_sent` idempotency map), student settings toggle UI for `fabrication_notify_email`.
- **Phase 2 next after (~2–3 days):** Python scanner worker on Fly.io. Requires fixtures bucketed + sidecar YAML first (Gate B) — fixtures currently an unsorted pile in `docs/projects/fabrication/fixtures/`. Worker reads `admin_settings.preflight.*` before dispatching Haiku enrichment; writes `ai_enrichment_cost_usd` + `thumbnail_views` per revision.
