...
| `PH8-FU-FAB-SCHOOL-SCOPING` | P1 | **Architectural follow-on to PH8-FU-SCHOOL-OWNED-FLEET** — Matt asked 25 Apr PM: "have you considered the fabricator account and how it interacts with school_id?" The honest answer was no — the original FU-P plan only covered labs + machines. Fabricators (lab techs, shared lab computers) are scoped per-teacher today via `fabricators.invited_by_teacher_id`. Under school-owned-fleet, that breaks — a school's lab tech needs to be manageable by every teacher at that school, not just the inviter. Migration 116 (25 Apr PM, applied alongside this) added the missing partial index on `fabricators.school_id` (the column itself was already reserved in mig 097, Phase 1A — surprise discovery). FU-P-2 flips the column to NOT NULL and rewires RLS. Resolves with FU-P. |
| `PH9-FU-MACHINE-CATEGORY-SVG` | P3 | Stylised SVG illustrations on machine cards — one for 3D printers, one for laser cutters (and future categories like vinyl cutters, CNC mills). Replaces the text-only card with something visual + memorable. ~30 min. Pure presentation; no schema change. Matt flagged 25 Apr PM: "wondering if i could have a picture of the 3d printer in the machine card?" |
| `PH9-FU-MACHINE-PHOTOS` | P3 | Per-machine custom photos. Two layers: (a) each system template ships with its own photo (Bambu X1C looks like a Bambu X1C); (b) teachers upload custom photos via the Edit modal for their own machines. Schema: `machine_profiles.image_path` (nullable text → Supabase Storage URL or static asset path). UI: photo replaces the category SVG when present. Image source for templates needs decision (manufacturer marketing photos = copyright issue; commission stylised illustrations as a house-style alternative). ~3 hours. |
| `PH9-FU-FILAMENT-LOADOUT` | P2 | Manual filament/material loadout tracking. Teacher updates "what's currently loaded" on each machine via the Edit modal — JSONB shape e.g. `{ "ams_slot_1": { "type": "PLA", "color": "#FF6B6B", "label": "Coral" }, "ams_slot_2": {...} }`. Surfaces as colour swatch chips on the machine card. Reuses OperationColorMapEditor pattern. Brand-agnostic — works for Bambu's AMS, Prusa MMU, lasers' loaded material, vinyl colours, etc. Schema: `machine_profiles.current_loadout` JSONB nullable. ~2 hours. Matt flagged 25 Apr PM: "AMS and colours? this would be the cherry on top when students are selecting." |
| `PH10-FU-BAMBU-MQTT-BRIDGE` | P3 | Live AMS state via Bambu's MQTT broker (reverse-engineered, undocumented). Companion app on school LAN subscribes to printer MQTT, pushes state to StudioLoom. Real engineering: 3-5 days, network access, credentials, firmware-version-fragile. Bambu-only (won't help xTool / Glowforge / Prusa). Park unless a school specifically asks for live state — manual loadout (PH9-FU-FILAMENT-LOADOUT) covers 95% of the value with 5% of the effort. References community projects: bambu-farm, bambulabs_api, Home Assistant Bambu integration. |
| `PH8-FU-SVG-BBOX-vs-ARTBOARD` | ✅ RESOLVED 26 Apr | **Fixed 26 Apr AM via Phase 8.1d-12.** R-SVG-01 was checking the SVG's stated `width`/`height` (artboard) against the laser bed instead of the actual cuttable geometry's bounding box. Matt smoke-test report: a 90mm coaster on a 250mm Inkscape "page" was getting a BLOCK against a Glowforge — the laser slicer would have only consumed the 90mm region. Fix: new `worker/svg_bbox.py` renders the SVG via cairosvg at ≤4096px and reads the alpha-channel bbox via PIL — robust against transforms, text font metrics, embedded images, gradients, and `<use>` references. R-SVG-01 prefers the content bbox; falls back to the artboard size when libcairo is unavailable or the render fails (over-report > silently pass). Evidence shape gained `bbox_source: "content"|"artboard"` + `artboard_mm` so the UI / debugging can tell which path fired. Existing fixture `box-oversized-10m.svg` — which had a 10m artboard with ~150×300mm of inner geometry — turned out to be the exact bug; sidecar metadata updated to `expected_result: "pass"` with calibration note. New tests: `test_r_svg_01_fires_on_truly_oversized_geometry` (inline 10m rect), `test_r_svg_01_does_not_fire_on_small_content_with_huge_artboard` (the smoke bug), `test_r_svg_01_artboard_fallback_when_render_fails` (cairo-missing path). |
| `PH9-FU-SVG-RENDER-CACHE` | P3 | The SVG scan now renders via cairosvg twice per job: once for the thumbnail (256×256), once for the R-SVG-01 content bbox (up to 4096px). Both reads are best-effort + already swallow errors, so duplication doesn't cause user-visible regressions — it just doubles the cairo work for SVG scans (typically 100–300ms per render). Optimisation: render once at the bbox resolution, downsample for the thumbnail. Or render once at the thumbnail resolution and use that lower-res surface for bbox (precision drops to ±1–2mm, acceptable for bed-fit). File until the SVG scan latency starts mattering — currently dwarfed by Fly cold-starts. |
| `PH9-FU-SVG-OPEN-PATH-ON-CUT` | **P1** | **Highest-impact SVG coverage gap.** A student draws a "closed" rectangle in Inkscape, but one corner is 0.05mm open. Visually identical, but the laser cuts a scratch line instead of a pierced piece — wastes material, demoralising, and completely silent today. Implementation: use `svgpathtools.parse_path(d)` then check whether `path[-1].end == path[0].start` (within float tolerance) for every path whose stroke colour maps to `cut`. Fire WARN per path, evidence: `{element_id, gap_mm}`. ~half-day. The "why didn't my piece cut out?" support ticket WILL come during pilot if this isn't shipped first. |
| `PH9-FU-SVG-FILL-ONLY-NO-STROKE` | **P1** | **Second-highest SVG gap.** Student does `<rect fill="red" stroke="none"/>` thinking the laser cuts the rectangle. R-SVG-04 (stroke-not-in-map) silently skips because there's no stroke at all — the rule was designed to flag *wrong* stroke colours, not missing strokes. Fire WARN: "this shape has fill but no stroke — lasers cut along strokes, not fills. Add a stroke colour from your operation map, or convert the fill to a stroke (Inkscape: Path > Stroke to Path)." Iterate `_iter_rendering_elements` (the existing helper), check `stroke_normalized is None` AND `fill_normalized is not None` AND fill colour maps to a cut/score operation in the profile. ~2 hours. |
| `PH9-FU-SVG-LAYER-SEMANTICS` | P2 | Laser convention is to put cut/score/engrave on named Inkscape layers (`<g inkscape:label="Cut">`). Current rules are 100% stroke-colour-driven — a student following the well-documented layer convention with the "wrong" colour gets punished. Fall back to layer label (`@inkscape:label` or `@id` matching `(?i)^(cut\|score\|engrave)`) when colour doesn't match the operation map. Effectively makes R-SVG-04 (stroke-not-in-map) less aggressive when the layer name is unambiguous. Needs decision on precedence (colour-wins vs label-wins) — Glowforge's slicer is colour-only, so colour-wins is safer. ~half-day. |
| `PH9-FU-SVG-USE-SYMBOL-EXPANSION` | P2 | Illustrator likes to emit `<use href="#piece"/>` referencing a `<symbol>` defined in `<defs>`. Current rules iterate `<path>` elements directly via `_iter_rendering_elements` and silently skip these — meaning an entire piece can pass every rule despite being malformed. Rare in Inkscape, common in Illustrator; significant blind spot for students using Illustrator school accounts. Implementation: pre-process the SvgDocument to expand `<use>` references inline (lxml DOM rewrite), or extend `_iter_rendering_elements` to follow `xlink:href` into `<defs>`. Latter is less destructive. ~1 day. |
| `PH8-FU-MACHINE-NAME-UNIQ-CONSTRAINT` | ✅ RESOLVED 26 Apr | **Fixed via migration 118 + 8.1d-13 commit.** Pre-118 the unique index `uq_machine_profiles_teacher_name` was scoped on `(teacher_id, name) WHERE teacher_id IS NOT NULL` — soft-deleted machines held names hostage AND two labs couldn't each own a "Bambu Lab P1S". Migration 118 rescopes to `(teacher_id, lab_id, name) WHERE teacher_id IS NOT NULL AND is_active = true` (strictly weaker, safe to apply against any data). Plus the API now probes for inactive duplicates on the 23505 path and surfaces "you have a deactivated machine called X — rename or reactivate" when that's the cause, vs "another machine in this lab is called X — pick a different name (e.g., 'X #2')" when it's a true active collision. Surfaced during Matt's Phase 8.1 S1 smoke 26 Apr: false-positive on Bambu Lab P1S add. |
| `PH9-FU-MACHINE-AUTOSUFFIX-NAME` | P3 | Smaller polish on top of the type-first picker (8.1d-13). When a teacher picks a template whose default name already exists in the target lab, pre-fill the name input with `{template.name} #2` (or `#3` etc., next free integer) so the user doesn't have to type the override themselves. Schools with 4 identical Bambu P1S printers in the same lab is a real case; this saves 4 manual renames. Implementation: pass `existingNamesInLab: string[]` from `LabSetupClient` → `AddMachineModal` → `MachineEditModal`; compute next-free suffix via a small helper. ~30 min. Deprioritised because migration 118 unsticks the immediate blocker — the auto-suffix is purely about keystroke savings. |
