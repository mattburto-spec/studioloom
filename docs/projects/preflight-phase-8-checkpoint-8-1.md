...
| `PH8-FU-CHILD-SCHOOL-SHARING` | P1 | Cross-teacher lab sharing via `fabrication_labs.school_id` reserved column. Blocked on FU-P access-model-v2 (school entity + school_memberships table + role enum). Migration path is pure `ADD POLICY` — schema is ready. |
