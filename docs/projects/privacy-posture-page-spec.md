...
**Status:** SPEC — drafted 2026-05-09
...
A single, scannable, public marketing-site page that documents exactly what StudioLoom collects about students, what it doesn't, what reaches third parties (especially AI providers), and how Anonymous Mode reduces the footprint further. The page exists to:

1. Give school IT directors, heads of school, and IB coordinators a single URL they can read in 5 minutes and forward to their compliance officer.
2. Serve as the linkable "see why" target for in-product trust signals (project workstream 4).
3. Be the foundation that the Vendor Approval Kit (workstream 3) and the data-flow diagram are derived from.
4. Force StudioLoom to actually inventory what it stores. The act of writing the page is half the value.

## Audience

**Primary:** school IT directors evaluating procurement, IB coordinators presenting tools to faculty, school heads signing contracts.
...
Linked prominently from the marketing footer ("How we handle student data") and from in-product trust signals (workstream 4).

## Page structure

Eight sections, top to bottom, scannable in five minutes:

### Section 1 — The principle (~80 words)
The one-paragraph version of the worldview. Approximate copy:

> StudioLoom is built so that schools can run a full design and inquiry curriculum without giving up student data they don't need to give up. Names, contact details, and dates of birth are optional. The platform works without them. AI mentors see handles, never real student names. This page documents exactly what we collect, what we don't, and where data goes.
...
Explain the option. When enabled at class creation, no student names, contact details, or DOB are stored on the server *at all*. Names live only on the teacher's browser, optionally exported as an encrypted file the teacher controls. Side-by-side comparison of standard vs anonymous: what's stored where in each.
...
### Section 7 — Sub-processors and data flow
...
### Section 8 — Your rights and our processes
Brief plain-language version: how a school requests data export, how a student is removed, retention defaults, breach notification process, contact for compliance questions. Link to the full DPA in the Vendor Approval Kit (workstream 3) when it exists.
...
1. **Pre-flight audit** (per above). Output: candidate data dictionary (markdown table), confirmed marketing-site routes, sub-processor reconciliation, drift notes.
...
3. **Draft page content** in markdown, sections 1-8. Submit to Matt for tone / accuracy review.
...
- The "what reaches AI providers" section has any factual ambiguity — that section especially must be precise. If it's not, stop and dig until it is.
