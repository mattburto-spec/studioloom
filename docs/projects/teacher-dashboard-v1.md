...
| 13d | **Class-frameworks registry cleanup** (~1 hour). Consolidate the duplicated framework arrays (`welcome/page.tsx`, `classes/page.tsx`, `classes/[classId]/page.tsx` — each inline-literal, plus 3 badge-color/label lookup objects per page) into one canonical `src/lib/frameworks/class-frameworks.ts` with `{ id, label, shortLabel, desc, color, bgTone, textTone, borderTone }`. `program.ts` reads from the same source. After this, adding a new framework = edit one array. | ⏳ Planned |
| 13e | **Inline "Assign unit" modal on class detail** (~2 hours). Currently the "Assign unit" button on `/teacher/classes/[classId]` navigates to `/teacher/units` — teacher loses context and has to navigate back. Replace with a modal that lists the teacher's existing units + a search field, one-click assigns, closes. Affects every class, not just PYP. | ⏳ Planned |
