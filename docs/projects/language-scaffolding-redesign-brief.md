...
- **`AutonomyPicker`** (Sub-Phase 3 of Lesson Bold, shipped this session):
  ...
  - Persists via **migration 116** → `student_progress.autonomy_level TEXT CHECK IN ('scaffolded','balanced','independent')`. Applied to local dev only (verified 24 Apr).
...
- ❌ **Dies.** Component file deleted. Helper functions (`hintsAvailable`, `exampleVisible`, etc.) deleted. `student_progress.autonomy_level` column dropped via migration 117. ActivityCard hint/example gating reverts to ELL-only logic (the pre-Sub-Phase-3 state).
...
**Drop:**
- `student_progress.autonomy_level` (rollback of migration 116)
...
- DELETE: `src/components/student/lesson-bold/__tests__/shell.test.tsx` autonomy + migration 116 test sections (8 tests removed).
- NEW: `supabase/migrations/117_drop_student_progress_autonomy_level.sql` — `ALTER TABLE student_progress DROP COLUMN IF EXISTS autonomy_level;`
...
**Riskiest assumption:** Migration 117 DROP COLUMN won't hit any currently-running query. Mitigation: applied to local dev only first; verify by hitting the lesson page + checking `/api/student/progress` POST works (the column is now gone but the retry pattern silently strips it from the payload).
...
- **`autonomy_level` column drop:** silent — no real data to migrate. Migration 116 was applied to local dev only; prod has never seen the column. If, somehow, prod has rows with `autonomy_level` set, they're discarded silently. No backup needed.
...
- Migration 117 DROP COLUMN attempted before AutonomyPicker code references are removed (PostgreSQL would error; just sequence the migration AFTER the code changes are committed)
