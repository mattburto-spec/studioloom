...
| **0.5** | **`user_profiles` table** (Option B chosen 28 Apr — separate table extending auth.users via FK, follows Supabase recommendation + existing `teachers` pattern). 6-value `user_type` enum: `student / teacher / fabricator / platform_admin / community_member / guardian`. `is_platform_admin BOOLEAN`. Trigger auto-creates row on `auth.users` INSERT. Backfill from existing `teachers`. | ~1 hr | 1 migration pair |
| **0.6** | Forward-compat tables — `school_resources` + `school_resource_relations`, `guardians` + `student_guardians`, `consents`, **`school_responsibilities` (programme coordinators), `student_mentors` (cross-program mentorship — resolves FU-MENTOR-SCOPE)** (schema only, no UX wiring) | ~4 hr | 3 migration pairs (collections / consents / coordinators+mentors) |
