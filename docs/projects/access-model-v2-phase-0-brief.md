...
| **0.3** | ✅ DONE — School-id gap fill (`students.school_id`, `units.school_id`) + backfill from teacher chain. Mig `20260428134250`. Tests +13. **NOT NULL tightening deferred to 0.8** (orphans need personal-school cascade first). | ~1 hr actual | 1 migration pair |
| **0.4** | ✅ DONE — Soft-delete (`deleted_at`) on `students` / `teachers` / `units` (3 cols) + `unit_version_id UUID FK unit_versions(id)` on 7 submission-shaped tables. Mig `20260428135317`. Tests +19. **Existing `is_archived` patterns on `classes` / `knowledge_items` / `activity_blocks` left alone — harmonisation deferred to Phase 6.** | ~45 min actual | 1 migration pair |
| **0.5** | ✅ DONE — `user_profiles` table (Option B): id PK REFERENCES auth.users(id) CASCADE + 6-value `user_type` enum (`student / teacher / fabricator / platform_admin / community_member / guardian`) + `is_platform_admin BOOLEAN`. Auto-create trigger on `auth.users` INSERT (alongside existing handle_new_teacher). Backfill from existing teachers. RLS: self-read + platform_admin-anywhere; INSERT/UPDATE deny-by-default (trigger + service role only). Mig `20260428142618`. Tests +20. | ~45 min actual | 1 migration pair |
| **0.6** | ✅ DONE — 7 forward-compat tables across 3 migration pairs. **0.6a** `school_resources` + `school_resource_relations` + `guardians` + `student_guardians` (mig `20260428214009`, +24 tests). **0.6b** `consents` (polymorphic subject, mig `20260428214403`, +16 tests). **0.6c** `school_responsibilities` (programme coordinators) + `student_mentors` (cross-program mentorship, polymorphic mentor via auth.users FK — resolves FU-MENTOR-SCOPE; mig `20260428214735`, +22 tests). | ~3 hr actual | 3 migration pairs |
| **0.7** | ✅ DONE — Core access tables across 2 migration pairs. **0.7a** `class_members` (6-value role enum incl. mentor) + `audit_events` (immutable, polymorphic actor_type 7 values, denorm school+class FKs, monetisation analytics seam, 5 indexes; mig `20260428215923`, +24 tests). **0.7b** `ai_budgets` (polymorphic subject student/class/school, UNIQUE override) + `ai_budget_state` (per-student running counter, due-reset partial index; mig `20260428220303`, +19 tests). RLS scoped reads throughout; INSERT/UPDATE deny-by-default; audit_events immutable by absence of mutation policies. | ~2 hr actual | 2 migration pairs |
| **0.8** | ✅ DONE — Split into 0.8a + 0.8b for safer manual application. **0.8a** (mig `20260428221516`, +18 tests) — single DO $$ block: orphan teachers → personal schools (FOR LOOP, RAISE EXCEPTION on remainder), students.school_id orphan tail, units.school_id orphan tail, class_members.lead_teacher seed (idempotent NOT EXISTS). RAISE NOTICE counters at every step + WARNING for classes.school_id NULLs. **0.8b** (mig `20260428222049`, +14 tests) — 3 ALTER COLUMN SET NOT NULL on students/units/classes school_id with pre-flight RAISE EXCEPTION guards. Apply 0.8a → review apply log → apply 0.8b only on clean state. | ~1.5 hr actual | 2 migration pairs (backfill / tighten) |
| **0.9** | ✅ DONE — 5 audit-derived non-schema deliverables: **🛡️ unknown-auth annotation pass** (scanner heuristic + 7 routes /api/tools/* → `auth: public`, gate threshold bumped 40→50; closes audit F10). **🛡️ Multi-Matt audit query** at `docs/security/multi-matt-audit-query.md` (read-only diagnostic, deferred to Matt's prod inspection). **🛡️ ENCRYPTION_KEY rotation script** at `scripts/security/rotate-encryption-key.ts` + procedure doc at `docs/security/encryption-key-rotation.md` (closes audit F9; per-row decrypt-encrypt-roundtrip-verify with --dry-run flag). **🛡️ MFA procedure doc** at `docs/security/mfa-procedure.md` (closes audit F6 procedurally; Matt enables in Supabase dashboard before pilot). **🛡️ Live RLS test harness scaffold** at `src/lib/access-v2/__tests__/rls-harness/` with setup helpers + 1 representative live test file (skipped if SUPABASE_TEST_URL not set; closes audit F14 partially, full coverage = `FU-AV2-RLS-HARNESS-FULL-COVERAGE` P2). | ~3 hr actual | Multiple — script + 3 docs + harness scaffold |
