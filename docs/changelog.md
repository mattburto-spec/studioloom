...
- **Sub-Phase 3: AutonomyPicker + migration 119 + ActivityCard hint/example gating (24 Apr).** Migration 119 added `student_progress.autonomy_level TEXT CHECK IN ('scaffolded','balanced','independent')` — no DEFAULT, no NOT NULL, no backfill (Lesson #38). 5 helpers gating hints + examples. Lesson #17 retry-without-column on both upsert paths. Tests 1944 → 1952 (+8). NC: flipped `hintsAvailable` to always-true, confirmed test failed at expected line, reverted via Edit (Lesson #41). Migration 119 applied to local dev only — scheduled for rollback in language-scaffolding-redesign Phase 0 (migration 120 DROP COLUMN).
...
- `supabase/migrations/119_student_progress_autonomy_level.sql` — applied dev only, scheduled for rollback
