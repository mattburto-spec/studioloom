...
FIXTURES: dict[str, dict] = {
    ...
    "known-good/stl/wheel-front-toy-vehicle.stl": {
        ...
        "triggers_rules": ["R-STL-13"],
        "notes": "Front wheel partner to back-wheel. 12 components — used to document upper bound of acceptable component count on a single print. Fires R-STL-13 (no flat base) because a wheel has a round base smaller than its bbox footprint — legitimate WARN that a student should acknowledge (brim helps).",
        "anonymised": False,
    },
    ...
    "known-good/stl/mount-bracket-130mm.stl": {
        ...
        "intended_machine": BAMBU,
        "expected_result": "warn",
        "triggers_rules": ["R-STL-11", "R-STL-13"],
        "notes": "Utility mount bracket, 130×60×35 mm, 23k faces, watertight single component. Largest clean fixture — good for asserting scan time on a realistic part. Fires R-STL-11 (overhangs) and R-STL-13 (small flat base relative to footprint) — both legit WARNs for an L-shape bracket that needs supports + brim. Student acknowledges and proceeds.",
        "anonymised": False,
    },
    ...
    "known-broken/stl/anon-chess-board-174-components.stl": {
        ...
        "intended_machine": BAMBU,
        "expected_result": "block",
        "triggers_rules": ["R-STL-01", "R-STL-04", "R-STL-11", "R-STL-13"],
        "notes": "Student chess board with 174 disconnected components (board + every piece as separate mesh). Violates single-print assumption. 175k faces. Also fires overhang + flat-base warnings because the individual pieces have round bases and chess-piece overhangs — these pile on the primary R-STL-01/04 blockers but don't change the block verdict.",
        "anonymised": True,
    },
    ...
    "known-broken/stl/chess-pawn-inverted-winding.stl": {
        ...
        "intended_machine": BAMBU,
        "expected_result": "block",
        "triggers_rules": ["R-STL-02", "R-STL-05"],
        "notes": "SYNTHETIC FIXTURE. Derived from chess-pawn-40mm.stl by reversing vertex winding on every other face. Primary target is R-STL-02 (is_winding_consistent=False). As a side-effect, reversing half the faces drives signed volume close to zero, so R-STL-05 also fires at our v1 epsilon of 0.01 mm³. Both rules firing is correct — the file is genuinely broken across two dimensions.",
        "anonymised": False,
    },
    "known-broken/stl/mount-bracket-oversized.stl": {
        ...
        "intended_machine": BAMBU,
        "expected_result": "block",
        "triggers_rules": ["R-STL-06", "R-STL-08", "R-STL-11", "R-STL-13"],
        "notes": "SYNTHETIC FIXTURE. Derived from mount-bracket-130mm.stl scaled 5×. Bbox 650×300×175 mm — exceeds Bambu X1C (256mm bed) on X and Y. R-STL-08 also fires because the scaled volume (~2.87M mm³) far exceeds the 120-min lab ceiling at v1 throughput of 15 mm³/s. R-STL-11 + R-STL-13 inherit from the base bracket (L-shape with small flat base).",
        "anonymised": False,
    },
    ...
    "borderline/stl/small-flat-sheet-1mm-thick.stl": {
        ...
        "expected_result": "pass",
        "triggers_rules": [],
        "notes": "Z=1.2 mm flat sheet. At v1 R-STL-09 threshold (<0.6 mm) passes — 1.2 mm is twice nozzle. At v1 R-STL-13 threshold (<10% flat-base ratio) passes — the whole part IS the flat base so ratio is 100%. Lives in borderline because the 'flat thin sheet' class of fixture is a natural prompt for both threshold-tuning conversations even when it currently emits nothing. Regression target: if R-STL-09 tightens to block at nozzle×3 (1.2 mm), this file would flip to WARN.",
        "anonymised": False,
    },
    "borderline/stl/anon-chess-board-repaired-4-components.stl": {
        ...
        "intended_machine": BAMBU,
        "expected_result": "block",
        "triggers_rules": ["R-STL-01", "R-STL-04", "R-STL-13"],
        "notes": "Repaired version of anon-chess-board-174-components.stl — the repair reduced 174 components to 4 but did NOT produce a fully watertight mesh (post-anonymisation-strip the file still has micro-gaps). R-STL-01 + R-STL-04 fire. R-STL-13 fires on the small per-piece flat base. File stays in borderline because it's the best example we have of an 'in-progress repair' state — useful for documenting that a partial fix isn't a full fix.",
        "anonymised": True,
    },
