 ...
 *
 * Run via: npx tsx scripts/backfill-activity-ids.ts
 *
 * This script:
 * 1. Reads all units.content_data
 * 2. For each page's sections, assigns nanoid(8) activityId where missing
 ...
 *
 * IMPORTANT: Run this BEFORE enabling drag-to-reorder in the lesson editor.
 */
...
interface ContentData {
  version?: number;
  pages?: Page[] | Record<string, { sections?: Section[] }>;
  timeline?: Array<{ id?: string; [key: string]: unknown }>;
  [key: string]: unknown;
}

function backfillContent(contentData: ContentData): { updated: boolean; sectionCount: number; backfilledCount: number } {
  let backfilledCount = 0;
  let sectionCount = 0;

  if (!contentData) return { updated: false, sectionCount: 0, backfilledCount: 0 };

  // v4 timeline — activities already have `id` field, but ensure it exists
  if (contentData.version === 4 && Array.isArray(contentData.timeline)) {
    for (const activity of contentData.timeline) {
      sectionCount++;
      if (!activity.id) {
        activity.id = nanoid(8);
        backfilledCount++;
      }
    }
  ...
  // v1 — pages as Record<string, PageContent>
  if (contentData.pages && typeof contentData.pages === "object" && !Array.isArray(contentData.pages)) {
    for (const pageKey of Object.keys(contentData.pages)) {
      const pageContent = contentData.pages[pageKey] as { sections?: Section[] };
...
async function migrateResponseKeys(unitId: string, contentData: ContentData) {
  // Build a mapping from section_N → activity_X for each page
  if (!Array.isArray(contentData.pages)) return 0;

  let migratedCount = 0;

  for (const page of contentData.pages as Page[]) {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (section.activityId) {
        const oldKey = `section_${i}`;
        const newKey = `activity_${section.activityId}`;
    ...
    // Find student_progress rows for this unit+page
    ...
    for (const row of progressRows) {
      ...
      for (const [key, value] of Object.entries(responses)) {
        if (keyMap[key] && !responses[keyMap[key]]) {
          // Migrate section_N to activity_X (keep old key too for safety)
...
async function main() {
  ...
  for (const unit of units || []) {
    ...
    if (updated) {
      ...
      if (error) {
        ...
        // Migrate response keys for this unit
  ...
  // 2. Process class_units.content_data (forks)
