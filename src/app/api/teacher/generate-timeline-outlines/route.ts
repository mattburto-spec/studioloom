...
import { TIMELINE_OUTLINE_SYSTEM_PROMPT, buildTimelineOutlinePrompt } from "@/lib/ai/prompts";
import { TIMELINE_OUTLINE_TOOL } from "@/lib/ai/schemas";
import {
  retrieveContext,
...
/**
 * POST /api/teacher/generate-timeline-outlines
 * Generate 3 distinct phase-based timeline outlines for the teacher to choose from.
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const userPrompt = buildTimelineOutlinePrompt(journeyInput, ragContext || undefined, teachingContextBlock);

    // Use tool-based generation if available
    if (provider.generateOutlines) {
      const result = await provider.generateOutlines(
        TIMELINE_OUTLINE_SYSTEM_PROMPT,
        userPrompt,
        TIMELINE_OUTLINE_TOOL
      );

      const options = (result as { options?: TimelineOutlineOption[] }).options;
      ...
      // Validate each option has phases
