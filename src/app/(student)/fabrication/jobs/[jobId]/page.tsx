 ...
 * label. Click the thumbnail to open a fullscreen lightbox modal
 * (PreviewModal) with click-to-toggle zoom.
 *
 * Falls back to a dashed placeholder box when the worker hasn't
 * produced a thumbnail yet (mid-scan or scan_error).
 *
 * Future (PH6-FU-PREVIEW-OVERLAY P2): annotate the thumbnail with
 * bounding boxes / highlights from each rule's `evidence` field
 * (face indices for STL, path coords for SVG) so students can see
 * WHERE the issue is, not just what it is. Data is already in
 * scan_results JSONB; just no UI layer for it yet.
 */
 ...
 *
 * Native-size mode uses an overflow-auto container so the user can
 * scroll around large images. For STL/SVG thumbnails (typically
 * 600–800px square from the scanner), this gives ~2× detail vs the
 * default fit-to-viewport view.
 *
 * Pinch-to-zoom + wheel-zoom + pan-while-zoomed are deferred — the
 * click-to-toggle covers the v1 use case (quick "let me see this
 * bigger") without a custom event handler. File as
 * PH6-FU-PREVIEW-PINCH-ZOOM if a teacher / student asks for it.
 */
function PreviewModal(props: {
  ...
  // Lock body scroll while modal is open. Restore on unmount so a
  // flash-unmount (e.g. fast close) doesn't leave the page stuck.
  React.useEffect(() => {
